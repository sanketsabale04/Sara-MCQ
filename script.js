...
const RECORDS_STORAGE_KEY = 'mocktest:records:v1';
const USER_STORAGE_KEY = 'mocktest:user:v1';
let startedAt = null;
...
// Replace the values below with your own Firebase Web App configuration.
// 1. Create a Firebase project at https://console.firebase.google.com
// 2. Add a Web App and copy the config values into firebaseConfig.
...
function initFirebase() {
    ...
    if (!window.firebase) {
        if (statusEl) statusEl.innerText = 'Firebase SDK not loaded.';
        return;
    }
    ...
    try {
        ...
        if (statusEl) statusEl.innerText = 'Cloud save enabled.';
    } catch (err) {
        if (statusEl) statusEl.innerText = 'Cloud save failed to initialize.';
...
function addTestRecord(record) {
    ...
    if (records.length > MAX_SAVED_RECORDS) {
        records.splice(MAX_SAVED_RECORDS);
    }
    saveRecords(records);
}
...
function loadSavedState() {
    ...
    if (typeof saved.submittedAt === 'number') submittedAt = saved.submittedAt;
    if (typeof saved.currentUser === 'string') currentUser = saved.currentUser;
...
function populateResults() {
    ...
    const durationMs = submittedAt ? (submittedAt - startedAt) : Math.max(0, (3 * 60 * 60 - timeLeft) * 1000);
    document.getElementById('result-details').innerHTML = `
        <div class="score-detail-card">
            ...
            <div class="score-detail-row"><span>Started</span><strong>${formatTimestamp(startedAt)}</strong></div>
            <div class="score-detail-row"><span>Submitted</span><strong>${formatTimestamp(submittedAt)}</strong></div>
            <div class="score-detail-row"><span>Time Taken</span><strong>${formatDuration(durationMs)}</strong></div>
        </div>
    `;
...
function renderSavedAttempts() {
    ...
    const records = getSavedRecords();
    if (!records.length) {
        container.innerHTML = '<div class="empty-history">No previous test records saved yet.</div>';
        return;
    }
        ...
        return `
            <div class="history-item">
                <div class="history-item-title">
                    <strong>${record.userName || 'Guest'}</strong>
                    <span>${formatTimestamp(record.submittedAt || record.startedAt)}</span>
                </div>
                <div class="history-item-body">
                    <div class="history-item-row"><span>Started</span><strong>${formatTimestamp(record.startedAt)}</strong></div>
                    <div class="history-item-row"><span>Ended</span><strong>${formatTimestamp(record.submittedAt)}</strong></div>
                    <div class="history-item-row"><span>Duration</span><strong>${record.durationLabel || formatDuration(record.duration || 0)}</strong></div>
                    <div class="history-item-row"><span>Marks</span><strong>${record.scoreSummary ? `${record.scoreSummary.marks}/720` : 'N/A'}</strong></div>
                    <div class="history-item-row"><span>Answered</span><strong>${record.answeredCount}</strong></div>
                    <div class="history-item-row"><span>Review</span><strong>${record.reviewCount}</strong></div>
                </div>
            </div>
        `;
    }).join('');
...
function renderWelcomeUser() {
    const userName = currentUser ? currentUser : 'Student';
    const userLabel = document.querySelector('.soft-hello');
    if (userLabel) userLabel.innerText = `Hello, ${userName}`;
}
...
function startTest() {
    ...
    if (startedAt === null) {
        const elapsedSeconds = 3 * 60 * 60 - timeLeft;
...
function downloadRecordedTest() {
    const record = {
        ...
        startTime: startedAt,
        endTime: submittedAt,
        durationMs: submittedAt ? submittedAt - startedAt : null,
        durationLabel: formatDuration(submittedAt ? submittedAt - startedAt : 0),
        formattedStartedAt: formatTimestamp(startedAt),
        formattedSubmittedAt: formatTimestamp(submittedAt),
        answeredCount: getOverallCounts().answered,
        reviewCount: getOverallCounts().review,
        unansweredCount: getOverallCounts().unanswered,
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `mock_test_record_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.json`;
    a.click();
}
...
function saveTestToCloud(record) {
    const statusEl = document.getElementById('cloud-status');
    if (!db) {
        if (statusEl) statusEl.innerText = 'Cloud database not initialized.';
        return Promise.reject(new Error('Cloud database is not initialized'));
    }

    return db.collection('testRecords')
        .add(record)
        .then(() => {
            if (statusEl) statusEl.innerText = 'Saved to cloud successfully.';
            console.log('Saved to cloud');
        })
        .catch(err => {
            if (statusEl) statusEl.innerText = 'Cloud save failed.';
...
function submitTest() {
    ...
    const duration = Math.max(0, submittedAt - startedAt);
    const overall = getOverallCounts();
    const scoreSummary = getScoreSummary();
    const record = {
        id: submittedAt,
        ...
        reviewCount: overall.review,
        unansweredCount: overall.unanswered,
        scoreSummary,
        answers: { ...userAnswers },
        reviewStatus: { ...reviewStatus },
        sections: sections.map(section => ({
